...
def add_textgrid_labels(textgrid_df, spec_plot, t1=None, t2=None):
    if t1 or t2:
        textgrid_df = textgrid_df[((textgrid_df['t1'] >= t1) & (textgrid_df['t1'] <= t2)) |
                                  ((textgrid_df['t2'] >= t1) & (textgrid_df['t2'] <= t2))].copy()
    ...
    for index, row in textgrid_df.iterrows():
        ...
        ax.annotate(row['phones'], (row['midpoint'], -500), textcoords="data", ha='center', fontsize='large')
        if row['words'] not in words:
            ax.annotate(row['words'], (row['word_mid'], -1500), textcoords="data", ha='center', fontsize='large')
